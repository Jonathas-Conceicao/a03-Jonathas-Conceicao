...
void test_too_few_blocks(){
    ...
    remove("grademe.bin");
    isEqual(inits("grademe.bin",5),FALHA,1);
...
void test_enough_blocks(){
    ...
    remove("grademe.bin");
    isEqual(inits("grademe.bin",500),SUCESSO,1);
}
...
void test_utf_8(){
    ...
    if (vfs){
        ...
        THEN("Deve criar");
        isGreaterThan(vopen(vfs, "teste", ESCRITA),0, 1);

        IF("Nome do arquivo tem .")
        THEN("Deve criar");
        isGreaterThan(vopen(vfs, "teste.txt", ESCRITA),0, 1);

        IF("Nome do arquivo contém UTF-8");
        THEN("Deve criar");
        isGreaterThan(vopen(vfs, "äß", ESCRITA),0, 1);

        IF("Nome do arquivo contém muitos UTF-8");
        THEN("Deve criar");
        isGreaterThan(vopen(vfs, "TODO", ESCRITA),0, 1);
    }
    else isEqual(0,1,4);
...
void test_multiple_files(){
	...
	for(int i=0; (i<255) && ok; i++){
		char buffer[10];
		snprintf(buffer, "%d", i);
		ok = ok && vopen(vfs, buffer, ESCRITA);
	}
